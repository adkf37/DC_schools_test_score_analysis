...
Sheets produced:
 ...
 16. School Sectors       — Avg proficiency, COVID recovery, and cohort growth by program sector
 17. School Needs         — Composite intervention-need score and tier per school
 18. Ward Analysis        — Avg proficiency, growth, and COVID recovery by DC ward (1–8)
 19. Equity Progress      — Achievement gap changes (first→last year) by subgroup pair
...
Dependencies:
    ...
    - output_data/school_sector_summary.csv    (produced by charter_dcps_analysis.py, optional)
    - output_data/school_needs_index.csv       (produced by school_needs_index.py, optional)
    - output_data/ward_summary.csv             (produced by ward_analysis.py, optional)
    - output_data/equity_progress_summary.csv  (produced by equity_progress_analysis.py, optional)
"""
import os
import sys
import pandas as pd
...
SCHOOL_SECTOR_FILE = os.path.join(OUTPUT_PATH, 'school_sector_summary.csv')
SCHOOL_NEEDS_INDEX_FILE = os.path.join(OUTPUT_PATH, 'school_needs_index.csv')
NEEDS_TIER_SUMMARY_FILE = os.path.join(OUTPUT_PATH, 'needs_tier_summary.csv')
WARD_SUMMARY_FILE = os.path.join(OUTPUT_PATH, 'ward_summary.csv')
REPORT_FILE = os.path.join(OUTPUT_PATH, 'summary_report.xlsx')
EQUITY_PROGRESS_SUMMARY_FILE = os.path.join(OUTPUT_PATH, 'equity_progress_summary.csv')
...
def _build_school_sector_sheet(school_sector: pd.DataFrame) -> pd.DataFrame:
    """
    School Program Sector Performance sheet.

    Shows average proficiency, COVID recovery metrics, and cohort growth
    for each program sector × subject combination.  Sectors are:
    Charter, DCPS Specialized, DCPS Alternative, DCPS Traditional.
    """
    if school_sector.empty:
        return pd.DataFrame()
    ...
    out = school_sector.rename(columns=rename)
    ...
    out['_sect_sort'] = out['School Sector'].map(sector_sort).fillna(99)
    out = out.sort_values(
        ['Subject', '_sect_sort']
    ).drop(columns=['_sect_sort']).reset_index(drop=True)
    return out


def _build_school_needs_sheet(needs_index: pd.DataFrame) -> pd.DataFrame:
    """
    School Needs Index sheet.

    Shows the composite intervention-need score and component scores for each
    school × subject combination.  Schools are sorted by composite needs score
    (descending) so the highest-need schools appear at the top.
    """
    if needs_index.empty:
        return pd.DataFrame()

    rename = {
        'School Name': 'School Name',
        'Subject': 'Subject',
        'composite_needs_score': 'Composite Needs Score (0–100)',
        'needs_tier': 'Needs Tier',
        'n_components': 'Components Used',
        'proficiency_need_score': 'Proficiency Need (0–100)',
        'growth_need_score': 'Growth Need (0–100)',
        'recovery_need_score': 'Recovery Need (0–100)',
        'equity_need_score': 'Equity Need (0–100)',
        'avg_proficiency_pct': 'Avg Proficiency (%)',
        'avg_pp_growth': 'Avg Cohort Growth (pp)',
        'net_vs_precovid_pp': 'Net vs Pre-COVID (pp)',
        'mean_abs_equity_gap': 'Mean Abs Equity Gap (pp)',
    }
    cols_in_df = [c for c in rename if c in needs_index.columns]
    out = needs_index[cols_in_df].rename(columns=rename)

    for col in ('Composite Needs Score (0–100)', 'Avg Proficiency (%)',
                'Avg Cohort Growth (pp)', 'Net vs Pre-COVID (pp)',
                'Mean Abs Equity Gap (pp)'):
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors='coerce').round(2)

    # Sort: ELA then Math, highest need first within each subject
    tier_order = ['Critical', 'High', 'Moderate', 'Low', 'Insufficient Data']
    tier_sort = {t: i for i, t in enumerate(tier_order)}
    out['_tier_sort'] = out.get('Needs Tier', pd.Series(dtype=str)).map(tier_sort).fillna(99)
    out = out.sort_values(
        ['Subject', '_tier_sort', 'Composite Needs Score (0–100)'],
        ascending=[True, True, False],
    ).drop(columns=['_tier_sort']).reset_index(drop=True)
    return out


def _build_ward_sheet(ward_summary: pd.DataFrame) -> pd.DataFrame:
    """
    Ward-Level Performance Analysis sheet.

    Shows average proficiency, cohort growth, and optional COVID recovery
    metrics for each DC ward × subject.  Wards are sorted numerically
    (1–8) within each subject group.
    """
    ...
    for col in ('Avg Proficiency (%)', 'Median Proficiency (%)',
                'Avg Cohort Growth (pp)', 'COVID Impact 2019→2022 (pp)',
                'Recovery 2022→2024 (pp)', 'Net vs Pre-COVID (pp)',
                'Gap vs Ward 3 (pp)'):
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors='coerce').round(2)

    out = out.sort_values(['Subject', 'Ward']).reset_index(drop=True)
    return out


def _build_equity_progress_sheet(equity_progress: pd.DataFrame) -> pd.DataFrame:
    """
    Equity Progress (Gap Closure) sheet.

    Shows how achievement gaps between key demographic pairs have changed
    from the earliest available year to the most recent year.  Negative
    gap_change_pp indicates that the gap narrowed (equity improved); positive
    values indicate the gap widened.  Sorted by Subject then gap_change_pp
    (narrowing gaps first).
    """
    if equity_progress.empty:
        return pd.DataFrame()

    rename = {
        'pair_label': 'Subgroup Pair',
        'reference_subgroup': 'Reference Subgroup',
        'comparison_subgroup': 'Comparison Subgroup',
        'Subject': 'Subject',
        'first_year': 'First Year',
        'last_year': 'Last Year',
        'gap_first_pp': 'Gap First Year (pp)',
        'gap_last_pp': 'Gap Last Year (pp)',
        'gap_change_pp': 'Gap Change (pp)',
        'gap_pct_change': 'Gap Change (%)',
        'direction': 'Direction',
        'n_schools_first': 'Schools (First Year)',
        'n_schools_last': 'Schools (Last Year)',
    }
    cols_in_df = [c for c in rename if c in equity_progress.columns]
    out = equity_progress[cols_in_df].rename(columns=rename)

    for col in ('Gap First Year (pp)', 'Gap Last Year (pp)',
                'Gap Change (pp)', 'Gap Change (%)'):
        if col in out.columns:
            out[col] = pd.to_numeric(out[col], errors='coerce').round(2)

    out = out.sort_values(
        ['Subject', 'Gap Change (pp)'], ascending=[True, True]
    ).reset_index(drop=True)
    return out
...
def main() -> None:
    ...
    # School sector summary is optional — generated by charter_dcps_analysis.py
    school_sector = pd.DataFrame()
    if os.path.isfile(SCHOOL_SECTOR_FILE):
        school_sector = pd.read_csv(SCHOOL_SECTOR_FILE)
        print(f'  Loaded school_sector_summary.csv: {len(school_sector):,} rows')
    else:
        print(f'  NOTE: {SCHOOL_SECTOR_FILE} not found — Sheet 16 will be skipped.')
    # School needs index is optional — generated by school_needs_index.py
    school_needs = pd.DataFrame()
    if os.path.isfile(SCHOOL_NEEDS_INDEX_FILE):
        school_needs = pd.read_csv(SCHOOL_NEEDS_INDEX_FILE)
        print(f'  Loaded school_needs_index.csv: {len(school_needs):,} rows')
    else:
        print(f'  NOTE: {SCHOOL_NEEDS_INDEX_FILE} not found — Sheet 17 will be skipped.')
    # Ward summary is optional — generated by ward_analysis.py
    ward_summary = pd.DataFrame()
    if os.path.isfile(WARD_SUMMARY_FILE):
        ward_summary = pd.read_csv(WARD_SUMMARY_FILE)
        print(f'  Loaded ward_summary.csv: {len(ward_summary):,} rows')
    else:
        print(f'  NOTE: {WARD_SUMMARY_FILE} not found — Sheet 18 will be skipped.')
    # Equity progress summary is optional — generated by equity_progress_analysis.py
    equity_progress = pd.DataFrame()
    if os.path.isfile(EQUITY_PROGRESS_SUMMARY_FILE):
        equity_progress = pd.read_csv(EQUITY_PROGRESS_SUMMARY_FILE)
        print(f'  Loaded equity_progress_summary.csv: {len(equity_progress):,} rows')
    else:
        print(f'  NOTE: {EQUITY_PROGRESS_SUMMARY_FILE} not found — Sheet 19 will be skipped.')
    print()
    ...
    school_sector_sheet = _build_school_sector_sheet(school_sector)
    school_needs_sheet = _build_school_needs_sheet(school_needs)
    ward_sheet = _build_ward_sheet(ward_summary)
    equity_progress_sheet = _build_equity_progress_sheet(equity_progress)
    ...
    if _has_openpyxl:
        ...
        # Sheet 16 — School Sectors (optional, requires charter_dcps_analysis.py)
        if not school_sector_sheet.empty:
            ws16 = wb.create_sheet('School Sectors')
            _write_df_to_sheet(ws16, school_sector_sheet,
                               growth_col_name='Avg Cohort Growth (pp)')
            _set_col_widths(ws16, [22, 10, 12, 20, 28, 28, 24, 26])
            _freeze_top_row(ws16)
            print(f'  ✓ Sheet 16: School Sectors — {len(school_sector_sheet)} rows')
        else:
            print('  – Sheet 16: School Sectors skipped '
                  '(run charter_dcps_analysis.py first)')

        # Sheet 17 — School Needs Index (optional, requires school_needs_index.py)
        if not school_needs_sheet.empty:
            ws17 = wb.create_sheet('School Needs')
            _write_df_to_sheet(ws17, school_needs_sheet,
                               growth_col_name='Composite Needs Score (0–100)')
            _set_col_widths(ws17, [45, 10, 26, 18, 16, 22, 20, 22, 20, 20, 24, 24, 26])
            _freeze_top_row(ws17)
            print(f'  ✓ Sheet 17: School Needs — {len(school_needs_sheet)} rows')
        else:
            print('  – Sheet 17: School Needs skipped '
                  '(run school_needs_index.py first)')

        # Sheet 18 — Ward Analysis (optional, requires ward_analysis.py)
        if not ward_sheet.empty:
            ws18 = wb.create_sheet('Ward Analysis')
            _write_df_to_sheet(ws18, ward_sheet, growth_col_name='Avg Cohort Growth (pp)')
            _set_col_widths(ws18, [8, 14, 10, 12, 20, 22, 24, 28, 26, 24, 20])
            _freeze_top_row(ws18)
            print(f'  ✓ Sheet 18: Ward Analysis — {len(ward_sheet)} rows')
        else:
            print('  – Sheet 18: Ward Analysis skipped '
                  '(run ward_analysis.py first)')

        # Sheet 19 — Equity Progress (optional, requires equity_progress_analysis.py)
        if not equity_progress_sheet.empty:
            ws19 = wb.create_sheet('Equity Progress')
            _write_df_to_sheet(ws19, equity_progress_sheet,
                               growth_col_name='Gap Change (pp)')
            _set_col_widths(ws19, [26, 32, 32, 10, 12, 12, 22, 22, 18, 16, 14, 22, 22])
            _freeze_top_row(ws19)
            print(f'  ✓ Sheet 19: Equity Progress — {len(equity_progress_sheet)} rows')
        else:
            print('  – Sheet 19: Equity Progress skipped '
                  '(run equity_progress_analysis.py first)')
